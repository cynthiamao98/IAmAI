...
//
    ...
    void CB_SearchTweets(bool success, string response)
    {
        if (success)
        {
            ...
            {
				if (tweet.retweeted_status.full_text != null)
                {
...
//                Debug.Log(trend.name);
    ...
    void CB_GetUserTimeline(bool success, string response)
    {
        if (success)
        {
			...
			System.IO.File.WriteAllText(Application.dataPath + "/Watson/TestData/" + handle + "_timeline.json", UserTimelineTweetsJSON);

			Debug.Log("Timeline file saved!");
